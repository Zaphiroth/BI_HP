...
server <- function(input, output, session) {
  ...
  observeEvent(input$summary, {
    if (grepl("pradaxa", tolower(input$summary))[1]) {
      updateSelectInput(session,
                        "category",
                        choices =  categorytype(),
                        selected = "SPAF")
    } else if (grepl("in hospital", tolower(input$summary))[1]) {
      updateSelectInput(session,
                        "category",
                        choices =  categorytype(),
                        selected = "In hospital")
    } else if (grepl("diabetes", tolower(input$summary))[1]) {
      updateSelectInput(session,
                        "category",
                        choices =  categorytype(),
                        selected = "DPP-IV")
    } else if (grepl("htn", tolower(input$summary))[1]) {
      updateSelectInput(session,
                        "category",
                        choices =  categorytype(),
                        selected = "ARB")
  ...
  ta <- reactive({
    # talist<-summary()$Category[!duplicated(summary()$Category)]
    talist1 <- summary()$Main[!duplicated(summary()$Main)]
    if ("Onco" %in% talist1 | "Diabetes" %in% talist1) {
      return(talist1)
    } else{
      return("Others")
    }
  ...
  observeEvent(input$summary, {
    updateSelectInput(session, "TA", selected = ta())
  })
  ...
  result2 <- reactive({
    ...
    result2_m_mat <- rmb1 %>%
      ...
      # summarise(final = sum(final, na.rm = TRUE)) %>%
      # ungroup() %>%
      filter((CORPORATE.DESC == "Overall" & Index == "GR%")|
               (CORPORATE.DESC == "B.INGELHEIM" & Index == "MS%") |
               (CORPORATE.DESC == "B.INGELHEIM" & Index == "GR%") |
               (CORPORATE.DESC == "B.INGELHEIM" & Index == "MS+/- %")) %>%
      unite(Index1, CORPORATE.DESC, Index) %>%
      spread(Index1, final)
    ...
    #   # summarise(final = sum(final, na.rm = TRUE)) %>%
    #   # ungroup() %>%
    #   arrange(AUDIT.DESC, desc(final)) %>%
    #   group_by(AUDIT.DESC) %>%
    #   mutate(rank = row_number()) %>%
    #   ungroup() %>%
    #   filter(CORPORATE.DESC == "B.INGELHEIM") 
    ...
    if (filename == "HTN") {
      ...
      if (filename == "Out hospital") {
        result5_m_mat <- rmb1 %>%
          filter(
            !(PRODUCT.DESC %in% c("ATROVENT B.I", "COMBIVENT B.I") & 
                CORPORATE.DESC == "B.INGELHEIM"),
            Category != "Overall",
            Index == "ABS",
            AUDIT.DESC == "China"
          ) %>%
          select(AUDIT.DESC, Category, CORPORATE.DESC, PRODUCT.DESC, 
                 Brand_CN, Index, final) %>%
          arrange(AUDIT.DESC, desc(final)) %>%
          group_by(AUDIT.DESC) %>%
          mutate(rank = row_number()) %>%
          ungroup() %>%
        ...
        result5_m_mat <- rmb1 %>%
          ...
          select(AUDIT.DESC, Category, CORPORATE.DESC, PRODUCT.DESC, 
                 Brand_CN, Index, final) %>%
          mutate(Brand_CN = ifelse(CORPORATE.DESC == "B.INGELHEIM",
                                   "BI Products",
                                   Brand_CN)) %>%
          group_by(AUDIT.DESC,
                   # Category, 
    ...
    if (ncol(result2_m_mat) == 2) {
      result2_m_mat <- result2_m_mat %>%
        mutate(`B.INGELHEIM_GR%` = NA,
               `B.INGELHEIM_MS%` = NA,
               `B.INGELHEIM_MS+/- %` = NA)
    }
    if (nrow(result5_m_mat) == 0) {
      result5_m_mat <- data.frame(rank = NA, stringsAsFactors = FALSE)
    }
    result2_m1_mat <- bind_cols(result2_m_mat, result5_m_mat["rank"]) %>%
      mutate(Period = "MAT") %>%
      select(Period, 
             `MKT GR vs. LY` = `Overall_GR%`, 
             `BI GR vs. LY` = `B.INGELHEIM_GR%`,
             MS = `B.INGELHEIM_MS%`,
             `MS+/- vs. LY` = `B.INGELHEIM_MS+/- %`,
             Rank = rank ) %>%
      mutate(`MKT GR vs. LY` = `MKT GR vs. LY` / 100,
             `BI GR vs. LY` = `BI GR vs. LY` / 100,
             MS = MS / 100,
             `MS+/- vs. LY` = `MS+/- vs. LY` / 100,
             Rank = Rank)
    
    ## for the rank and brand # and manufacture type ms
    manu_type_map <- rmb1 %>%
      select(CORPORATE.DESC, MANUF.TYPE.DESC) %>%
      mutate(MANUF.TYPE.DESC = ifelse(MANUF.TYPE.DESC == "JOINT-VENTURE",
                                      "IMPORT",
                                      MANUF.TYPE.DESC)) %>%
      distinct() %>%
      filter(MANUF.TYPE.DESC != "Overall") %>%
      arrange(CORPORATE.DESC, MANUF.TYPE.DESC)
    ...
    result3_m <- rmb1 %>%
      ...
      summarise(index = n_distinct(CORPORATE.DESC)) %>%
      ungroup() %>%
    ...
    if (ncol(result2_m_ytd) == 2) {
      result2_m_ytd <- result2_m_ytd %>%
        mutate(`B.INGELHEIM_GR%` = NA,
               `B.INGELHEIM_MS%` = NA,
               `B.INGELHEIM_MS+/- %` = NA)
    }
    
    if (nrow(result5_m_ytd) == 0) {
      result5_m_ytd <- data.frame(rank = NA, stringsAsFactors = FALSE)
    }
    
    
    result2_m1_ytd <- bind_cols(result2_m_ytd, result5_m_ytd["rank"]) %>%
      mutate(Period = "YTD") %>%
      select(Period, 
             `MKT GR vs. LY` = `Overall_GR%`, 
             `BI GR vs. LY` = `B.INGELHEIM_GR%`,
    ...
    if (ncol(result2_m_qtr) == 2) {
      result2_m_qtr <- result2_m_qtr %>%
        mutate(`B.INGELHEIM_GR%` = NA,
               `B.INGELHEIM_MS%` = NA,
               `B.INGELHEIM_MS+/- %` = NA)
    }
    
    if (nrow(result5_m_qtr) == 0) {
      result5_m_qtr <- data.frame(rank = NA, stringsAsFactors = FALSE)
    }
    
    result2_m1_qtr <- bind_cols(result2_m_qtr, result5_m_qtr["rank"]) %>%
      mutate(Period = "QTR") %>%
      select(Period, 
             `MKT GR vs. LY` = `Overall_GR%`, 
             `BI GR vs. LY` = `B.INGELHEIM_GR%`,
    ...
    if (nrow(result5_m_mth) == 0) {
      result5_m_mth <- data.frame(rank = NA, stringsAsFactors = FALSE)
    }
    
    result2_m1_mth <- bind_cols(result2_m, result5_m_mth["rank"]) %>%
      mutate(Period = "MTH") %>%
      select(Period, 
             `MKT GR vs. LY` = `Overall_GR%`, 
             `BI GR vs. LY` = `B.INGELHEIM_GR%`,
    ...
    list(summary_table = bind_rows(result2_m1_mat, 
                                   result2_m1_ytd,
                                   result2_m1_qtr,
                                   result2_m1_mth),
         summary_table1 = bind_rows(result3_m, 
                                    result4_m) %>%
           select(Metrics, Index))
  })
